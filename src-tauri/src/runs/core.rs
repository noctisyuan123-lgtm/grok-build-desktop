...
impl AcpHost {
    ...
    /// In-place ACP rewind on an already-connected host (warm Undo path).
    /// File restore via shadow-git is intentionally *not* awaited here: the
    /// Desktop IPC layer / FE owns workspace restore so Undo does not block on
    /// disk I/O. ConversationOnly still applies ACP `file_snapshots` when All
    /// was rejected.
    pub(crate) async fn rewind_last_user_turn(
        ...
        // Shadow-git / workspace restore is owned by `rewind_grok_session` +
        // FE `restore_workspace_on_undo` (redo-stash). Do not await disk I/O on
        // this hot path — that was freezing Undo behind a cold ACP restart.
        Ok(RewindResult {
            rewound: true,

...
describe('UsageMeter', () => {
  ...
  it('compares remaining quota with remaining time on the same scale', () => {
    ...
    expect(screen.getByRole('meter', { name: '68% quota remaining' })).toBeInTheDocument();
    expect(
      screen.getByRole('img', { name: /68% quota remaining; 90% of the billing window remaining/ }),
    ).toBeInTheDocument();
    expect(document.querySelector('.set-quota-fill.quota.tone-yellow')).toBeTruthy();
    expect(screen.getByText('Reset in 6d 8h')).toBeInTheDocument();
  ...
  it('turns the quota fill orange at 30% remaining and red at 10%', () => {
    ...
    expect(document.querySelector('.set-quota-fill.quota.tone-orange')).toBeTruthy();
    ...
    expect(document.querySelector('.set-quota-fill.quota.tone-red')).toBeTruthy();
